...
class BasesfSimpleBlogActions extends sfActions
{
  ...
  {
    $this->post_pager = DbFinder::from('sfSimpleBlogPost')->
      recent()->
      paginate($this->getRequestParameter('page', 1), sfConfig::get('app_sfSimpleBlog_post_max_per_page', 5));
  }

  public function executePostsFeed()
  {
    sfLoader::loadHelpers(array('I18N'));
    $posts = DbFinder::from('sfSimpleBlogPost')->
      recent()->
      find($this->getRequestParameter('nb', sfConfig::get('app_sfSimpleBlog_feed_count', 5)));
  ...
  {
    sfLoader::loadHelpers(array('I18N'));
    $comments = DbFinder::from('sfSimpleBlogComment')->
      recent()->
      find($this->getRequestParameter('nb', sfConfig::get('app_sfSimpleBlog_feed_count', 5)));
  ...
  {
    sfLoader::loadHelpers(array('I18N'));
    $post = DbFinder::from('sfSimpleBlogPost')->findByStrippedTitleAndDate(
  ...
  {
    $this->post = DbFinder::from('sfSimpleBlogPost')->findByStrippedTitleAndDate(
      $this->getRequestParameter('stripped_title'),
      $this->getDateFromRequest()
    );
